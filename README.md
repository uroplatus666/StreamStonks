...
## 💰 Бизнес-ценность
В условиях волатильности рынка розничные инвесторы часто принимают эмоциональные решения, теряя капитал.
**StreamStonks** решает эту проблему, предоставляя:
1.  **Объективную аналитику:** Прогнозы строятся на ML-модели (CatBoost), обученной на 5 годах исторических данных тех. анализа.
2.  **Экономию времени:** Автоматический скрининг десятков тикеров за секунды вместо часов ручного анализа.
3.  **Снижение рисков:** Система подсвечивает тренды (Рост/Падение), помогая фильтровать заведомо убыточные сделки.

---

## 🏗 Архитектура и MLOps фичи
Проект реализован как набор микросервисов с асинхронным взаимодействием:

* **Event-Driven Inference:** Пользовательский интерфейс не блокируется. Запросы летят в **Kafka**, обрабатываются ML-воркером, а результат пушится в БД.
* **Zero-Downtime Retraining:** Планировщик (`scheduler`) переобучает модель каждые 12 часов. Воркер (`ml_worker`) автоматически подхватывает новую модель без перезапуска ("Hot Swap").
* **Observability:** Полный мониторинг здоровья системы и качества данных через **Grafana**.

### 🛠 Технический стек
* **ML Core:** CatBoost, Pandas, Scikit-learn
* **Infrastructure:** Docker, Docker Compose
* **Messaging:** Kafka, Zookeeper
* **Database:** PostgreSQL (хранение фичей, логов и прогнозов)
...
<summary><h2>🧠 Подробная логика работы</h2>
(Нажмите, чтобы развернуть)</summary>
Проект реализует **полный цикл MLOps (End-to-End)**: от сбора сырых данных до инференса и мониторинга в реальном времени. Архитектура построена на принципе микросервисов, общающихся через базу данных и брокер сообщений.

### 1. Data Pipeline (Сбор данных) `ingest/ingest_worker.py`
* **Логика:**
...
### 2. ML Training  `ml/scheduler.py` + `ml/train_model.py`
Модели далеко до хорошей, по-сути это baseline, который можно дораатывать в будущем, добавляя sentiment analysis и т.д.
...
* **Versioning:** Новая модель сохраняется в `shared volume/`. Воркер предсказаний (`ml/ml_worker.py`) автоматически подхватывает обновленный файл модели без перезапуска контейнера (**Hot Swap**).

### 3. Event-Driven Inference (Прогнозы)

Система использует асинхронную архитектуру для обработки запросов пользователей:

1. **Запрос:** Пользователь выбирает тикер и горизонт прогноза в **Streamlit** (от 1 до 14 дней).
2. **Очередь:** UI не блокируется, а отправляет задачу (JSON) в топик **Kafka** (`predict_requests`).
3. **Обработка:** `ml/ml_worker.py` вычитывает сообщение из Kafka.
...
5. **Результат:** Прогноз записывается в БД, и Streamlit отображает его пользователю

### 4. Monitoring. **Grafana** подключена напрямую к PostgreSQL.
...
Дашборд предназначен для мониторинга производительности ML-сервиса и состояния данных.
...
```
<details>
<summary><h2>📈Скриншоты визуала в Streamlit UI</h2>
(Нажмите, чтобы развернуть)</summary>
...
<details>
<summary><h2>📊Скиншоты части панелей в Grafana</h2>
(Нажмите, чтобы развернуть)</summary>

![example-1](images/grafana1.jpg)
__________________________________________________

![example-1](images/grafana2.jpg)
__________________________________________________
